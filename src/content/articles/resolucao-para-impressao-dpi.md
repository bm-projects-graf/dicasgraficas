...
No Adobe Photoshop:
Abre a imagem
`Imagem → Tamanho da imagem` (`Image → Image Size`)
Vê o campo Resolução — este é o teu DPI
Confirma também o Tamanho do documento (em centímetros ou polegadas) — é o tamanho real a que a imagem será impressa àquela resolução
...
Verifica a resolução de cada imagem ao tamanho final em que vai ser impressa
Mantém 300 DPI para tudo que seja visualizado de perto
Usa vetoriais sempre que possível — especialmente para logótipos, texto e ilustrações
Não estiques imagens pequenas — procura sempre o original em alta qualidade
